...
class TTTOptimizationCalculator:
    ...
    def calculate_optimal_ttt(self, lead_data: Dict, ego_speed: float, target_speed: float) -> Tuple[float, float]:
        ...
        # Optimized: Use coarser steps to reduce computation in 20Hz loop
        # ✅ PERFORMANCE: 2.0 km/h steps instead of 0.5 km/h for 4x fewer iterations
        max_boost_20pct = min(self.max_overtaking_boost, ego_speed * 0.20)
        boost_step = 2.0 * CV.KPH_TO_MS  # 2.0 km/h steps (was 0.5) for better performance
        boost_levels = [i * boost_step for i in range(int(max_boost_20pct / boost_step) + 1)]
        # Early exit optimization: limit to maximum 10 iterations
        boost_levels = boost_levels[:10]
