...
def manager_init() -> None:
  ...
  default_params: list[tuple[str, str | bytes]] = [
    ...
    # ========================================================================
    # APOM (Accelerator Pedal Override Mode) - sunnypilot MADS Compatible
    # ========================================================================
    ("EnableAPOM", "0"),                          # APOM enable/disable (default: disabled)
    ("AccAPOMCombo", "1"),                        # ACC+APOM combo mode (enable APOM when ACC enabled)
    ("APOMCruiseMain", "1"),                      # APOM main toggle (tied to cruise availability)
  ...
  # Always reset security-critical settings to defaults on every reboot
  ...
  params.putBool("np_opom_enabled", False)  # OPOM disabled on reboot (security default)

  # DCP Migration: Handle existing AEM/ACM users
  migrate_legacy_to_dcp(params)
...
def migrate_legacy_to_dcp(params: Params) -> None:
  """Migrate existing AEM/ACM users to DCP system."""

  # Check if this is a fresh installation
  migration_done = params.get_bool("np_dcp_migration_done", False)
  if migration_done:
    return

  cloudlog.info("[DCP Migration] Checking for legacy AEM/ACM configuration...")

  # Check legacy parameters from common.py or older configurations
  # We check for any indication the user was using AEM or ACM
  legacy_aem_enabled = False
  legacy_acm_enabled = False

  # Check if user has any non-default longitudinal settings indicating they used advanced features
  try:
    # Look for evidence of AEM usage (conservative approach)
    # If user has any advanced longitudinal parameters set, they likely used AEM
    existing_dcp_mode = params.get_int("np_dcp_mode", 1)

    # For now, we'll use a conservative migration strategy:
    # - If DCP mode is already set to non-default, assume user chose it intentionally
    # - Otherwise, migrate conservatively to Highway mode for existing users

    if existing_dcp_mode == 1:  # Default highway mode
      # Check if this appears to be an existing nagaspilot installation
      has_np_params = any(params.get(key) is not None for key in [
        "np_device_enabled", "np_device_last_log"
      ])

      if has_np_params:
        # Existing nagaspilot user - assume they had some longitudinal features enabled
        # Migrate to DCP mode (adaptive) for full compatibility
        params.put("np_dcp_mode", "3")  # DCP/Adaptive mode
        cloudlog.info("[DCP Migration] Existing user migrated to DCP Mode 3 (Adaptive)")
      else:
        # New installation - keep Highway mode default
        cloudlog.info("[DCP Migration] New installation - using Highway mode default")

  except Exception as e:
    cloudlog.warning(f"[DCP Migration] Migration check failed: {e}")
    # Safe fallback - keep current settings

  # Mark migration as completed
  params.put_bool("np_dcp_migration_done", True)
  cloudlog.info("[DCP Migration] Migration completed")
