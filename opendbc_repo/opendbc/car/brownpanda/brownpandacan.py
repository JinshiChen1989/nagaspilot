...
def create_long_command(packer, CAN: CanBusBase, CC, CS, accel: float, torque_command: float, counter: int = 0):
  ...
  # DBC scaling: brake pressure uses 0.015625 scale, brake pedal uses 0.4 scale
  brake_pressure = abs(min(accel, 0.0)) / 0.015625  # Convert to DBC units
  brake_pedal = min(abs(min(accel, 0.0)) / 0.04, 102.0)  # 0.4 scale factor in DBC, max 102%
