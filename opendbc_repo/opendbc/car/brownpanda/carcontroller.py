...
class CarController(CarControllerBase):
  ...
  def __init__(self, dbc_name, CP, VM):
    ...
    self.lat_counter = 0            # latCommand counter
    self.long_counter = 0           # longCommand counter  
    self.long2_counter = 0          # longCommand2 counter
    self.disp_counter = 0           # dispCommand counter
    self.vision_counter = 0         # visionInfo counter
    ...
    # Initialize DTSA controller for supported BYD models (DragonPilot pattern)
    self.dtsa_controller = None
    if DTSA_AVAILABLE and hasattr(self.params, 'DTSA_ENABLE') and self.params.DTSA_ENABLE:
      try:
        self.dtsa_controller = NPDTSAController(self.CP, self.params)
      except Exception as e:
        print(f"DTSA Controller initialization failed: {e}")
        self.dtsa_controller = None
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if CC.latActive:
      ...
      # Apply DTSA enhancements for supported BYD models (DragonPilot pattern)
      if self.dtsa_controller is not None:
        limited_torque = self.dtsa_controller.apply_dtsa_enhancements(limited_torque, CS)
        # Update DTSA live parameters
        self.dtsa_controller.update_live_parameters()
  ...
  def _generate_latCommand_message(self, steering_angle, steering_torque, CC):
    # Delegate value building to brownpandacan with counter
    msg = brownpandacan.create_lat_command(self.packer, self.CAN, CC, steering_angle, steering_torque, self.lat_counter)
  ...
  def _generate_longCommand_message(self, CC, CS, accel, torque_command):
    ...
    # Delegate value building to brownpandacan with counter
    msg = brownpandacan.create_long_command(self.packer, self.CAN, CC, CS, accel, torque_command, self.long_counter)
    self.long_counter = (self.long_counter + 1) & 0xF  # Increment and wrap at 16
    return [msg]
  ...
  def _generate_longCommand2_message(self, CC, CS, accel):
    ...
    # Delegate value building to brownpandacan with counter
    msg = brownpandacan.create_long_command2(self.packer, self.CAN, CC, CS, accel, self.stopping_counter, self.params, self.long2_counter)
    self.long2_counter = (self.long2_counter + 1) & 0xF  # Increment and wrap at 16
    return [msg]
  ...
  def _generate_visionInfo_message(self, CC, CS):
    ...
    # Delegate value building to brownpandacan with counter
    msg = brownpandacan.create_vision_info(self.packer, self.CAN, CC, CS, self.vision_counter)
    self.vision_counter = (self.vision_counter + 1) & 0xF  # Increment and wrap at 16
    return [msg]
  ...
  def _generate_dispCommand_message(self, CC, CS):
    ...
    # Delegate value building to brownpandacan with counter
    msg = brownpandacan.create_disp_command(self.packer, self.CAN, CC, CS, self.disp_counter)
    self.disp_counter = (self.disp_counter + 1) & 0xF  # Increment and wrap at 16
    return [msg]
