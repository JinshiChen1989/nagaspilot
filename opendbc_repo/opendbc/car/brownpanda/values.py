...
class BrownPandaCarSpecs(CarSpecs):
...
class BrownPandaPlatformConfig(PlatformConfig):
  dbc_dict: DbcDict = field(default_factory=lambda: {Bus.pt: 'brownpanda'})
...
class CarControllerParams:
  ...
  def _init_byd_atto3_params(self):
    ...
    # DTSA parameters for ATTO3 (DragonPilot pattern)
    self.DTSA_ENABLE = True                # DTSA available for this model
    self.DTSA_MAX_TORQUE = 240             # Max torque with DTSA
    self.DTSA_RESPONSE_FACTOR = 1.1        # Response scaling factor
  ...
  def _init_byd_dolphin_params(self):
    ...
    # DTSA parameters for DOLPHIN (DragonPilot pattern)
    self.DTSA_ENABLE = True                # DTSA available for this model  
    self.DTSA_MAX_TORQUE = 210             # Max torque with DTSA
    self.DTSA_RESPONSE_FACTOR = 0.95       # Comfort-focused response scaling
  ...
  def _init_deepal_s05_params(self):
    ...
    # DTSA not supported for DEEPAL (focus on BYD only)
    self.DTSA_ENABLE = False               # DTSA not available for this model
  ...
  def _init_default_params(self):
    ...
    # DTSA not supported for unknown models
    self.DTSA_ENABLE = False               # DTSA not available for unknown models
...
class CAR(Platforms):
  BYD_ATTO3 = BrownPandaPlatformConfig(
    ...
    BrownPandaCarSpecs(mass=1680, wheelbase=2.72, steerRatio=16.0, minSteerSpeed=3 * CV.KPH_TO_MS)
  )
  
  DEEPAL_S05 = BrownPandaPlatformConfig(
    [BrownPandaCarDocs("DEEPAL S05")],
    BrownPandaCarSpecs(mass=1750, wheelbase=2.72, steerRatio=15.5, minSteerSpeed=3 * CV.KPH_TO_MS)
  )
  
  BYD_DOLPHIN = BrownPandaPlatformConfig(
    [BrownPandaCarDocs("BYD DOLPHIN")],
    BrownPandaCarSpecs(mass=1405, wheelbase=2.70, steerRatio=14.5, minSteerSpeed=5 * CV.KPH_TO_MS)
  )
